...
SELECT 	COUNT (nbre_films_vus.nofilm)
FROM 
	films nbre_films_vus
	JOIN voir v ON nbre_films_vus.nofilm = v.nofilm
	JOIN individus i ON v.nospect = i.nopers;


/* 2. Numéros et noms des acteurs qui ont joué plus de 3 films (strictement). */
SELECT (noacteur, nompers)
FROM
	individus i
...
FROM 
	films f
	JOIN jouer j ON j.nofilm = f.nofilm
	JOIN aimer a ON a.nospect = j.nospect
WHERE a.nospect > 10;
...
FROM 
	voir v
	JOIN films f ON v.nofilm = f.nofilm
